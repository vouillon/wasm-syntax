...
module Encoder = struct
  ...
  let rec instr b (i : 'info instr) =
    ...
    | If { typ; if_block; else_block; _ } ->
        byte b 0x04;
        (match typ with Some t -> blocktype b t | None -> byte b 0x40);
        List.iter (instr b) if_block;
        if else_block <> [] then (
          byte b 0x05;
          List.iter (instr b) else_block);
        byte b 0x0B
    | Br i ->
        byte b 0x0C;
        uint b i
...
let output_byte ch b = Out_channel.output_byte ch b
let output_string ch s = Out_channel.output_string ch s
...
let module_ ?(color = Utils.Colors.Auto) ?(out_channel = stdout)
    (m : 'info module_) =
  let _ = color in
  output_string out_channel "\x00\x61\x73\x6D\x01\x00\x00\x00";
  ...
  if m.tables <> [] then
    output_section out_channel 4
      (Encoder.vec (fun b (t : 'info table) -> Encoder.limits b t.typ))
      m.tables;
  ...
  if m.globals <> [] then
    output_section out_channel 6
      (Encoder.vec (fun b (g : 'info global) ->
           Encoder.globaltype b g.typ;
           List.iter (Encoder.instr b) g.init;
           Encoder.byte b 0x0B))
      m.globals;
  ...
  if m.elem <> [] then
    output_section out_channel 9
      (Encoder.vec (fun b (e : 'info elem) ->
         match e.mode with
         | Active (table, offset) ->
             Encoder.byte b 0x06;
             Encoder.uint b table;
             List.iter (Encoder.instr b) offset;
             Encoder.byte b 0x0B;
             Encoder.reftype b e.typ;
             Encoder.vec (fun b ex -> List.iter (Encoder.instr b) ex; Encoder.byte b 0x0B) b e.init
  ...
  (* Fields are part of types, handled by type name or by struct itself *)
  ...
  if Buffer.length b_names > 0 then (
    ...
    output_byte out_channel 0; (* Custom section ID (0) *)
    let len = Buffer.length b_custom_section_content in
    (* Use local uint encoder that writes directly to out_channel *)
    let rec output_uint i =
      if i < 128 then output_byte out_channel i
      else (
        output_byte out_channel (128 + (i land 127));
        output_uint (i lsr 7))
    in
    output_uint len;
