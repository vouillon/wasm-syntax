...
and instruction ch =
  ...
  let desc =
    ...
    | 0xFD -> (
        ...
        | 154 -> VecBinOp (VecAvgr `I16)
        | 155 -> VecBinOp VecQ15MulrSat
        ...
        | 236 -> VecUnOp (VecCeil `F32)
        | 237 -> VecUnOp (VecFloor `F32)
        | 238 -> VecUnOp (VecTrunc `F32)
        | 239 -> VecUnOp (VecNearest `F32)
        | 240 -> VecUnOp (VecAbs F64x2)
        | 241 -> VecUnOp (VecNeg F64x2)
        | 243 -> VecUnOp (VecSqrt `F64)
        | 244 -> VecBinOp (VecAdd F64x2)
        | 245 -> VecBinOp (VecSub F64x2)
        | 246 -> VecBinOp (VecMul F64x2)
        | 247 -> VecBinOp (VecDiv `F64)
        | 248 -> VecBinOp (VecMin (None, F64x2))
        | 249 -> VecBinOp (VecMax (None, F64x2))
        | 250 -> VecBinOp (VecPMin `F64)
        | 251 -> VecBinOp (VecPMax `F64)
        | 252 -> VecUnOp (VecCeil `F64)
        | 253 -> VecUnOp (VecFloor `F64)
        | 254 -> VecUnOp (VecTrunc `F64)
        | 255 -> VecUnOp (VecNearest `F64)
        (* Relaxed SIMD *)
