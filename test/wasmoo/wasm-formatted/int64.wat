...
(func $int64_hash (param $v (ref eq)) (result i32)
  (local $i i64)
  (local.set $i (struct.get $int64 1 (ref.cast (ref $int64) (local.get $v))))
  ...
  (loop $loop
    ...
    (if (i32.lt_s (local.get $i) (local.get $len))
      (then
        ...
        (local.set $res
          (i64.add
            (i64.mul (local.get $res) (i64.extend_i32_u (local.get $base)))
  ...
  (if (local.get $signedness)
    (then
      (if (i32.gt_s (local.get $sign) (i32.const 0))
        (then
          (if
            (i64.ge_u (local.get $res) (i64.shl (i64.const 1) (i64.const 63)))
            (then (call $caml_failwith (local.get $errmsg)))))
        (else
          (if
            (i64.gt_u (local.get $res) (i64.shl (i64.const 1) (i64.const 63)))
            (then (call $caml_failwith (local.get $errmsg))))))))
...
(func (export "caml_int64_format")
  ...
  (local.set $uppercase)
  (local.set $base (i64.extend_i32_u))
  (local.set $signed)
  (local.set $alternate)
