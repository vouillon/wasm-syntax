...
(type $closure_last_arg
  (sub $closure (struct (field (ref (;(field i32);) $function_1))))
)
(type $function_2
...
(type $closure_4
  (sub $closure (struct (field (ref $function_1)) (field (ref $function_4))))
)
...
(func (export "caml_is_last_arg") (param $v (ref eq)) (result i32)
...
(func $caml_obj_dup (export "caml_obj_dup")
  ...
  (drop
    (block $not_block (result (ref eq))
      (local.set $orig
        (br_on_cast_fail $not_block (ref eq) (ref $block) (local.get 0)))
...
(func (export "caml_lazy_make_forward") (param (ref eq)) (result (ref eq))
  (array.new_fixed $block 2 (ref.i31 (global.get $forward_tag)) (local.get 0))
)
...
(func $obj_update_tag
  ...
  (if (result i32)
    (ref.eq (array.get $block (local.get $b) (i32.const 0))
      (ref.i31 (local.get $o)))
    (then
      (array.set $block (local.get $b) (i32.const 0) (ref.i31 (local.get $n)))
      (i32.const 1))
    (else (i32.const 0)))
)
...
(func (export "caml_lazy_reset_to_lazy") (param (ref eq)) (result (ref eq))
  (drop
    (call $obj_update_tag (local.get 0) (global.get $forcing_tag)
      (global.get $lazy_tag)))
  (ref.i31 (i32.const 0))
)

(func (export "caml_lazy_update_to_forward")
  (param (ref eq)) (result (ref eq))
  (drop
    (call $obj_update_tag (local.get 0) (global.get $forcing_tag)
...
(func (export "caml_lazy_update_to_forcing")
  ...
  (if (ref.test (ref $block) (local.get 0))
    (then
      (if
        (call $obj_update_tag (local.get 0) (global.get $lazy_tag)
...
(func (export "caml_obj_compare_and_swap")
  (param (ref eq)) (param (ref eq)) (param $old (ref eq))
...
(func (export "caml_get_public_method")
  ...
  (local $meths (ref $block)) (local $tag i32) (local $cacheid i32)
