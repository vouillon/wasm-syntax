...
(type $float (struct (field f64))) (type $block (array (mut (ref eq))))
...
(global $ERROR_DETECTED i32 (i32.const 5)) (global $loop i32 (i32.const 6))
...
(global $tbl_defred i32 (i32.const 6)) (global $tbl_dgoto i32 (i32.const 7))
(global $tbl_sindex i32 (i32.const 8)) (global $tbl_rindex i32 (i32.const 9))
...
(func $token_name
  ...
  (loop $loop
    (if (i32.eqz (array.get_u $string (local.get $names) (local.get $i)))
      (then
        (return
          (array.new_data $string $unknown_token (i32.const 0) (i32.const 15)))))
...
(func $print_token
  ...
  (if (ref.test (ref i31) (local.get $tok))
    ...
    (else
      ...
      (call $output
        (call $token_name
          (array.get $block (local.get $tables) (global.get $tbl_names_block))
          (i31.get_u
            (ref.cast (ref i31)
...
(func (export "caml_parse_engine")
  ...
  (local $m i32) (local $state1 i32) (local $sp i32) (local $asp i32)
  (local $state i32) (local $errflag i32) (local $tables (ref $block))
  (local $env (ref $block)) (local $cmd i32) (local $arg (ref $block))
  (local $tbl_defred (ref $string)) (local $tbl_sindex (ref $string))
  (local $tbl_check (ref $string)) (local $tbl_rindex (ref $string))
  (local $tbl_table (ref $string)) (local $tbl_len (ref $string))
  (local $tbl_lhs (ref $string)) (local $tbl_gindex (ref $string))
    ...
    (loop $next
      (block $default
                    ...
                    (block $ERROR_DETECTED
                      (block $testshift
                        (block $TOKEN_READ
                          (block $loop
                            (block $START
                              (br_table $START $TOKEN_READ $STACKS_GROWN_1
                                $STACKS_GROWN_2 $SEMANTIC_ACTION_COMPUTED
                                $ERROR_DETECTED $loop $testshift $shift
                                $shift_recover $reduce $default
                                (local.get $cmd)))
                            ;; START:
                        ...
                        (block $cont
                          ...
                          (array.set $block (local.get $env)
                            ...
                            (array.get $block
                              ...
                              (i32.add
                                (i31.get_u
                                  (ref.cast (ref i31) (local.get $varg)))
                                (i32.const 1))))
                          (array.set $block (local.get $env)
                            (global.get $env_lval) (ref.i31 (i32.const 0))))
                          ...
                          (i31.get_s
                            (ref.cast (ref i31)
                              (array.get $block (local.get $env)
                                (global.get $env_curr_char))))))
                      (if
                        (i32.and (i32.ne (local.get $n1) (i32.const 0))
                          (i32.ge_s (local.get $n2) (i32.const 0)))
                        (then
                          (if
                            (i32.le_s (local.get $n2)
                              (i31.get_s
                                (ref.cast (ref i31)
                                  (array.get $block (local.get $tables)
        ...
        (array.set $block
          ...
          (i32.add (local.get $sp) (i32.const 1)) (local.get $varg))
        (local.set $asp
          (i31.get_s
            (ref.cast (ref i31)
