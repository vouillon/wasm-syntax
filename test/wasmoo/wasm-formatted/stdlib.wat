(module
  ...
  (func $find_named_value
    (param $s (ref eq)) (param $l (ref null $assoc))
    (result (ref null $assoc))
    (local $a (ref $assoc))
  ...
  (func (export "caml_register_named_value")
    ...
    (local.set $h
      (i32.rem_u
        (i31.get_s
          (ref.cast (ref i31)
            (call $caml_string_hash (ref.i31 (i32.const 0)) (local.get 0))))
        (global.get $Named_value_size)))
    (local.set $r
  ...
  (func $caml_main (export "caml_main") (param $start (ref func))
    (local $exn (ref eq))
    (try
      (do (drop (call_ref $func (ref.cast (ref $func) (local.get $start)))))
      (catch $ocaml_exit (call $exit (pop i32)))
