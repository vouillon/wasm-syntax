...
(func $pop_compare_stack
  ...
  (local.set $v1
    (array.get $block_array (struct.get $compare_stack 1 (local.get $stack))
      (local.get $i)))
  (local.set $v2
    (array.get $block_array (struct.get $compare_stack 2 (local.get $stack))
      (local.get $i)))
  ...
  (tuple.make 2 (array.get $block (local.get $v1) (local.get $p))
...
(func $push_compare_stack
  ...
  (local.set $i
    (i32.add (struct.get $compare_stack 0 (local.get $stack)) (i32.const 1)))
  (local.set $len
        ...
        (block $heterogeneous (result (ref eq))
          ...
          (if (call $caml_is_closure (local.get $v1))
            (then
              ...
              (call $clear_compare_stack)
              (call $caml_invalid_argument
                (array.new_data $string $functional_value (i32.const 0)
                  (i32.const 25)))))
