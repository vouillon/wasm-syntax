...
(func $apply_continuation (param $resolver (ref extern)) (param $v (ref eq))
  ...
  (return_call_ref $called_with_continuation
    (struct.new $cont_resume (ref.func $invoke_promise_resolver)
      (local.get $resolver))
    (struct.get $thunk 1 (local.get $t)) (struct.get $thunk 0 (local.get $t)))
)
...
(func $dummy_fun (param (ref eq) (ref eq)) (result (ref eq)) (unreachable))
...
(func $push_stack
  (param $stack (ref $fiber)) (param $k (ref $cont)) (result (ref $cont))
  (block $done
    (loop $loop
      (global.set $stack
        (struct.new $fiber (struct.get $fiber $handlers (local.get $stack))
          (local.get $k) (global.get $stack)))
      (local.set $k (struct.get $fiber $cont (local.get $stack)))
      (local.set $stack
        (br_on_null $done (struct.get $fiber $next (local.get $stack))))
      (br $loop)))
  (local.get $k)
)
...
(func $identity (param (ref eq) (ref eq)) (result (ref eq)) (local.get 0))
...
(func $trampoline_iterator
  ...
  (return_call_ref $function_2
    ...
    (if (result (ref eq))
      ...
      (else (local.get $env)))
    (local.get $f)
    (struct.get $cps_closure 0 (ref.cast (ref $cps_closure) (local.get $f))))
)
...
(func $dummy_cps_fun (param (ref eq) (ref eq) (ref eq)) (result (ref eq))
  (unreachable)
)
