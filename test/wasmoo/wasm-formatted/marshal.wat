(module
  ...
  (func $readstr (param $s (ref $intern_state)) (result (ref $string))
    ...
    (array.copy $string $string (local.get $res) (i32.const 0)
      (local.get $src) (local.get $pos) (local.get $len))
    (struct.set $intern_state $pos (local.get $s)
  ...
  (func $readfloat
    ...
    (local $src (ref $string)) (local $pos i32) (local $res i32)
    (local $d i64) (local $i i32) (local $v (ref eq))
    (local.set $src (struct.get $intern_state $src (local.get $s)))
    (local.set $pos (struct.get $intern_state $pos (local.get $s)))
    (struct.set $intern_state $pos (local.get $s)
  ...
  (func (export "caml_deserialize_int_8") (param $vs (ref eq)) (result i64)
    ...
    (i64.or
      (i64.shl (i64.extend_i32_u (call $read32 (local.get $s)))
        (i64.const 32))
                      ...
                      (if
                        ...
                        (else
                          (block $INT8
                            ...
                            (br $done))
                          (local.set $v
                            (ref.i31 (call $read8s (local.get $s))))
                          (br $done))))))
                (local.set $ofs
                  (i32.sub
  ...
  (func $parse_header
    ...
    (drop (call $read32 (local.get $s)))
    (struct.new $marshal_header (local.get $data_len)
  ...
  (func $reserve_extern_output
    ...
    (if (struct.get $extern_state $user_provided_output (local.get $s))
      (then
        (call $caml_failwith
          (array.new_data $string $buffer_overflow (i32.const 0)
            (i32.const 34)))))
  ...
  (func $writefloats
    ...
    (local $pos i32) (local $sz i32) (local $buf (ref $string))
    (local $d i64) (local $i i32) (local $j i32)
  ...
  (func $extern_shared_reference
    (param $s (ref $extern_state)) (param $d i32)
  ...
  (func $extern_string
    (param $s (ref $extern_state)) (param $v (ref $string))
    (local $len i32)
    ...
    (loop $loop
      (block $next_item
        ...
        (if (call $caml_is_closure (local.get $v))
          (then
            (call $caml_invalid_argument
              (array.new_data $string $func_value (i32.const 0)
                (i32.const 30)))))
        (if (call $caml_is_continuation (local.get $v))
          (then
            (call $caml_invalid_argument
              (array.new_data $string $cont_value (i32.const 0)
                (i32.const 32)))))
  ...
  (func (export "caml_output_value_to_string")
    ...
    (local $sz i32) (local $buf (ref $string))
    (local $st (ref $extern_state)) (local $blk (ref $output_block))
    (local $pos i32) (local $len i32) (local $res (ref $string))
    (local.set $blk
    ...
    (local.set $res
      (array.new $string (i32.const 0)
        (i32.add (local.get $sz) (i32.const 20))))
    (array.copy $string $string (local.get $res) (i32.const 0)
  ...
  (func (export "caml_output_value")
    ...
    (local $sz i32) (local $buf (ref $string))
    (local $st (ref $extern_state)) (local $blk (ref $output_block))
    (local $len i32) (local $res (ref $string))
    (local.set $blk
      (struct.new $output_block (ref.null $output_block)
        (global.get $SIZE_EXTERN_OUTPUT_BLOCK)
        (array.new $string (i32.const 0)
    ...
    (local.set $buf)
    (local.set $sz)
    (call $caml_really_putblock (local.get $ch) (local.get $buf)
      (i32.const 0) (i32.const 20))
    (loop $loop
      (block $done
        (local.set $len (struct.get $output_block $end (local.get $blk)))
