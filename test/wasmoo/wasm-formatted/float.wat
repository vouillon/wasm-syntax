...
(global $infinity (ref $chars)
  (array.new_fixed $chars 8 (i32.const 105) (i32.const 110) (i32.const 102)
    (i32.const 105) (i32.const 110) (i32.const 105) (i32.const 116)
    (i32.const 121))
)

(global $nan (ref $chars)
  (array.new_fixed $chars 3 (i32.const 110) (i32.const 97) (i32.const 110))
)
...
(func (export "caml_hexstring_of_float")
  ...
  (local.set $s
    (block $sign (result (ref $string))
      ...
      (if
        (i32.and (i32.ge_s (local.get $prec) (i32.const 0))
        ...
        (then
          ...
          (local.set $half (i64.shr_u (local.get $unit) (i64.const 1)))
          ...
          (if
            (i32.or (i64.gt_u (local.get $frac) (local.get $half))
...
(global $inf (ref $chars)
  (array.new_fixed $chars 3 (i32.const 105) (i32.const 110) (i32.const 102))
)
          ...
          (local.set $txt
            (if (result (ref $chars)) (i64.eqz (local.get $m))
              ...
              (else
                ...
                (local.set $i (i32.ne (local.get $sign_style) (i32.const 0)))
                (global.get $nan))))
          (local.set $len (array.len (local.get $txt)))
          (local.set $s
            (array.new $string (i32.const 0)
      ...
      (if (i32.lt_u (local.get $i) (local.get $len))
        (then
          ...
          (if (i32.eq (local.get $c) (i32.const 46)) ;; '.'
            (then
              (br_if $error (i32.ge_s (local.get $dec_point) (i32.const 0)))
              (local.set $dec_point (local.get $n_bits))
              (br $parse)))
          (if
            (i32.or
              (i32.eq (local.get $c) (i32.const 80)) ;; 'P'
              (i32.eq (local.get $c) (i32.const 112)))
            ;; 'p'
            (then
                ...
                (loop $parse_exponent
                  ...
                  (local.set $exp
                    (i32.add (i32.mul (local.get $exp) (i32.const 10))
                      (local.get $d)))
                  (br_if $overflow (i32.lt_u (local.get $exp) (local.get $d)))
          ...
          (if
            (i32.and (i32.ge_u (local.get $c) (i32.const 48))
              (i32.le_u (local.get $c) (i32.const 57)))
            (then (local.set $d (i32.sub (local.get $c) (i32.const 48))))
            (else
              (if
                (i32.and (i32.ge_u (local.get $c) (i32.const 97))
                  (i32.le_u (local.get $c) (i32.const 102)))
                (then (local.set $d (i32.sub (local.get $c) (i32.const 87))))
                (else
                  (if
                    ...
                    (else (br $error)))))))
          (local.set $n_bits (i32.add (local.get $n_bits) (i32.const 4)))
          (br_if $parse
          ...
          (if (i32.lt_u (local.get $m_bits) (i32.const 60))
            (then
              ...
              (local.set $m_bits (i32.add (local.get $m_bits) (i32.const 4))))
            (else
              (if (local.get $d)
...
(func (export "caml_float_of_string") (param (ref eq)) (result (ref eq))
        ...
        (if (i32.lt_u (local.get $i) (local.get $len))
          (then
            ...
            (if (i32.ne (local.get $c) (i32.const 95)) ;; '_'
              (then
                (array.set $string (local.get $s') (local.get $j)
                  (local.get $c))
                (local.set $j (i32.add (local.get $j) (i32.const 1)))))
            (br $copy))))
      (local.set $len (array.len (local.get $s')))
  ...
  (block $error
    ...
    (if (i32.eq (i32.add (local.get $i) (i32.const 3)) (local.get $len))
      (then
        (local.set $c (array.get_u $string (local.get $s) (local.get $i)))
        (if (i32.eq (i32.and (local.get $c) (i32.const 0xdf)) (i32.const 78))
          (then
            ;; 'N'
            (local.set $i (i32.add (local.get $i) (i32.const 1)))
            (local.set $c (array.get_u $string (local.get $s) (local.get $i)))
              ...
              (then
                ...
                (if
                  (i32.eq (i32.and (local.get $c) (i32.const 0xdf))
                    (i32.const 78))
                  ;; 'N'
                  (then (return (struct.new $float (f64.const nan)))))))))
        (if (i32.eq (i32.and (local.get $c) (i32.const 0xdf)) (i32.const 73))
          (then
            ;; 'I'
            (local.set $i (i32.add (local.get $i) (i32.const 1)))
            (local.set $c (array.get_u $string (local.get $s) (local.get $i)))
...
(func (export "caml_nextafter_float")
  ...
  (if (f64.eq (local.get $x) (local.get $y)) (then (return (local.get $y))))
  (if (result f64) (f64.eq (local.get $x) (f64.const 0))
    ...
    (else
      ...
      (if
        (i32.and (i64.lt_s (local.get $i) (local.get $j))
...
(func (export "caml_frexp_float") (param (ref eq)) (result (ref eq))
  ...
  (call $frexp (struct.get $float 0 (ref.cast (ref $float) (local.get 0))))
  (local.set $e)
  (local.set $m)
  (array.new_fixed $block 3 (ref.i31 (i32.const 0))
...
(func (export "caml_fma_float")
  ...
  (result (ref eq))
  (local $x f64) (local $y f64) (local $z f64) (local $3 i64) (local $4 i64)
  (local $5 i64) (local $6 i64) (local $7 i64) (local $8 i64) (local $9 i32)
  (local $10 i32) (local $11 f64) (local $12 f64) (local $13 f64)
  (local $14 f64) (local $15 f64)
  (local.set $x (struct.get $float 0 (ref.cast (ref $float) (local.get $vx))))
  (local.set $y (struct.get $float 0 (ref.cast (ref $float) (local.get $vy))))
  (local.set $z (struct.get $float 0 (ref.cast (ref $float) (local.get $vz))))
  (local.set $7
    (i64.add
      (local.tee $4
        (i64.and
          (i64.shr_u (local.tee $3 (i64.reinterpret_f64 (local.get $y)))
            (i64.const 52))
          (i64.const 2047)))
      (local.tee $6
        (i64.and
          (i64.shr_u (local.tee $5 (i64.reinterpret_f64 (local.get $x)))
            (i64.const 52))
          (i64.const 2047)))))
  ...
  (block $label$1
    (block $label$2
      ...
      (br_if $label$2
        (i64.gt_u (i64.and (local.get $8) (i64.const 0x7fe0000000000000))
      ...
      (br $label$1))
    (local.set $8
      (i64.and (i64.shr_u (local.get $8) (i64.const 52)) (i64.const 2047)))
    (block $cont
      ...
      (return
        (struct.new $float
          (f64.add (f64.add (local.get $x) (local.get $z)) (local.get $y)))))
    (block $cont
    ...
    (block $cont
      ...
      (local.set $y
        (select (f64.const 0x1p-1074) (f64.const -0x1p-1074)
          (i64.gt_s (i64.xor (local.get $3) (local.get $5)) (i64.const -1))))
      (block $cont2
        (br_if $cont2 (i64.lt_u (local.get $8) (i64.const 3)))
        (return (struct.new $float (f64.add (local.get $y) (local.get $z)))))
      (return
        (struct.new $float
          (f64.mul
      ...
      (block $label$11
        (block $label$12
          ...
          (local.set $z
            (select (f64.mul (local.get $z) (f64.const 0x1p-53))
              (local.get $z) (i64.gt_u (local.get $8) (i64.const 53))))
          (local.set $x
          ...
          (local.set $y
            (select (local.get $y)
              (f64.mul (local.get $y) (f64.const 0x1p-53)) (local.get $9)))
          (br $label$11))
        ...
        (block $label$13
          (block $label$14
            ...
            (block $label$15
              (br_if $label$15 (i64.le_u (local.get $6) (local.get $4)))
          ...
          (block $label$16
            ...
            (local.set $x
              (select (f64.mul (local.get $x) (f64.const 0x1p-53))
                (local.get $x) (i64.gt_u (local.get $6) (i64.const 53))))
            (br $label$13))
          (local.set $y
            (select (f64.mul (local.get $y) (f64.const 0x1p-53))
              (local.get $y) (i64.gt_u (local.get $4) (i64.const 53)))))
    ...
    (block $label$17
      ...
      (local.set $x
        (select (f64.mul (local.get $x) (f64.const 0x1p108)) (local.get $x)
          (local.tee $9 (i64.gt_u (local.get $6) (local.get $4)))))
      (local.set $y
  ...
  (block $cont
    ...
    (br_if $cont
      (i32.eqz
        (i32.or (f64.eq (local.get $y) (f64.const 0))
      ...
      (f64.mul
        (local.tee $12
          (f64.sub (local.get $x)
            (local.tee $11
              (f64.sub
                (local.tee $11 (f64.mul (local.get $x) (f64.const 0x8000001)))
                (f64.sub (local.get $11) (local.get $x))))))
        (local.tee $14
          (f64.sub (local.get $y)
            (local.tee $13
              (f64.sub
                (local.tee $13 (f64.mul (local.get $y) (f64.const 0x8000001)))
                (f64.sub (local.get $13) (local.get $y)))))))
      (f64.sub
        (f64.sub
          (f64.sub (local.tee $15 (f64.mul (local.get $y) (local.get $x)))
  ...
  (block $label$21
    (block $label$22
      (br_if $label$22
        (f64.ne (local.tee $y (f64.add (local.get $z) (local.get $15)))
          (f64.const 0)))
                          ...
                          (f64.add
                            (local.tee $11
                              (f64.add
                                ...
                                (f64.sub (local.get $z)
                                  (f64.sub (local.get $y) (local.get $11)))))
                            (local.get $x)))
                        (local.get $11))))
                  ...
                  (f64.add
                    (f64.sub (local.get $z)
                      (local.tee $13
                        (f64.sub
                          (local.tee $x
                            (f64.add (local.get $y) (local.get $z)))
                          (local.get $y))))
                    (f64.sub (local.get $y)
                      (f64.sub (local.get $x) (local.get $13)))))
                (local.tee $y (f64.add (local.get $11) (local.get $y))))))
          (f64.const 0)))
      (br_if $cont
        (i32.and
          (i32.wrap_i64 (local.tee $4 (i64.reinterpret_f64 (local.get $y))))
          (i32.const 1)))
      (local.set $y
        (f64.reinterpret_i64
          (i64.add
