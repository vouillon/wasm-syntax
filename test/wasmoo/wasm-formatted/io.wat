...
(import "bindings" "map_get"
  (func $map_get (param (ref extern) i32) (result (ref $fd_offset)))
)
...
(func $initialize_fd_offset (param $fd i32) (param $offset i64)
  (call $map_set (call $get_fd_offsets) (local.get $fd)
...
(global $sys_open_flags (ref $open_flags)
  (array.new_fixed $open_flags 9 (i32.const 1) (i32.const 2) (i32.const 6)
    (i32.const 8) (i32.const 16) (i32.const 32) (i32.const 0) (i32.const 0)
    (i32.const 64))
)
...
(func (export "caml_ml_out_channels_list") (param (ref eq)) (result (ref eq))
...
(func (export "caml_ml_open_descriptor_in")
  ...
  (struct.new $channel (global.get $channel_ops) (call $custom_next_id)
    ...
    (call $ta_new (global.get $IO_BUFFER_SIZE)) (i32.const 0) (i32.const 0)
    (global.get $IO_BUFFER_SIZE) (i32.const 0))
)

(global $caml_stderr (export "caml_stderr") (mut (ref eq))
  (ref.i31 (i32.const 0))
)
...
(func (export "caml_ml_close_channel") (param (ref eq)) (result (ref eq))
  (local $ch (ref $channel)) (local $fd i32)
...
(func $caml_refill (param $ch (ref $channel)) (result i32)
  ...
  (if (i32.eqz (local.get $n)) (then (call $caml_raise_end_of_file)))
...
(func $caml_getblock (export "caml_getblock")
  (param $vch (ref eq)) (param $s (ref $string)) (param $pos i32)
  (param $len i32) (result i32)
  (local $ch (ref $channel)) (local $avail i32) (local $nread i32)
  ...
  (if (local.get $avail)
    (then
      ...
      (call $ta_blit_to_string (struct.get $channel $buffer (local.get $ch))
  ...
  (call $ta_blit_to_string (struct.get $channel $buffer (local.get $ch))
    (i32.const 0) (local.get $s) (local.get $pos) (local.get $len))
    ...
    (then
      (if (i32.gt_u (local.get $avail) (i32.const 0))
        ...
        (else
          ...
          (if (i32.gt_u (local.get $len) (local.get $nread))
            (then (local.set $len (local.get $nread))))))))
  (call $ta_blit_to_string (local.get $buf) (local.get $curr) (local.get $s)
    (local.get $pos) (local.get $len))
  (struct.set $channel $curr (local.get $ch)
...
(func $caml_getch (param $ch (ref $channel)) (result i32)
  ...
  (return_call $ta_get_ui8 (struct.get $channel $buffer (local.get $ch))
    (local.get $curr))
)

(func (export "caml_ml_input_char") (param $ch (ref eq)) (result (ref eq))
  (ref.i31 (call $caml_getch (ref.cast (ref $channel) (local.get $ch))))
)

(func (export "caml_ml_input_int") (param $vch (ref eq)) (result (ref eq))
  (local $ch (ref $channel)) (local $res i32)
  (local.set $ch (ref.cast (ref $channel) (local.get $vch)))
  (local.set $res (i32.shl (call $caml_getch (local.get $ch)) (i32.const 24)))
  (local.set $res
...
(func (export "caml_ml_pos_in") (param $vch (ref eq)) (result (ref eq))
...
(func (export "caml_ml_pos_in_64") (param $vch (ref eq)) (result (ref eq))
  (local $ch (ref $channel))
  (local.set $ch (ref.cast (ref $channel) (local.get $vch)))
...
(func (export "caml_ml_pos_out") (param $vch (ref eq)) (result (ref eq))
  (local $ch (ref $channel))
  (local.set $ch (ref.cast (ref $channel) (local.get $vch)))
  (ref.i31
    (i32.add
...
(func (export "caml_ml_pos_out_64") (param $vch (ref eq)) (result (ref eq))
  (local $ch (ref $channel))
  (local.set $ch (ref.cast (ref $channel) (local.get $vch)))
  (call $caml_copy_int64
    (i64.add
...
(func $caml_seek_in
  ...
  (if
    (i32.and
      (i64.ge_s (local.get $dest)
        (i64.sub (local.get $offset)
          (i64.extend_i32_s (struct.get $channel $max (local.get $ch)))))
...
(func (export "caml_ml_seek_in")
  ...
  (return_call $caml_seek_in (ref.cast (ref $channel) (local.get $ch))
    (i64.extend_i32_s (i31.get_s (ref.cast (ref i31) (local.get $dest)))))
)
      ...
      (then
        (if (struct.get $channel $curr (local.get $ch))
          (then
            ...
            (call $ta_copy (struct.get $channel $buffer (local.get $ch))
        ...
        (if
          (i32.ge_u (struct.get $channel $max (local.get $ch))
            (struct.get $channel $size (local.get $ch)))
          (then
            (return
              (ref.i31
        ...
        (local.set $n
          (call $caml_do_read (local.get $ch)
            ...
            (i32.sub (struct.get $channel $size (local.get $ch))
              (struct.get $channel $max (local.get $ch)))))
...
(func $caml_flush_partial (param $ch (ref $channel)) (result i32)
  ...
  (if (i32.gt_u (local.get $towrite) (i32.const 0))
    (then
            ...
            (if (result i32)
              ...
              (then
                (call $write (local.get $fd) (local.get $buf) (i32.const 0)
                  (local.get $towrite) (local.get $offset)))
              (else
                (call $write' (local.get $fd) (local.get $buf) (i32.const 0)
      ...
      (if (i32.gt_u (local.get $towrite) (i32.const 0))
        (then
          (call $ta_copy (local.get $buf) (i32.const 0) (local.get $written)
            (local.get $towrite))))
...
(func $caml_putblock
  ...
  (local.set $buf (struct.get $channel $buffer (local.get $ch)))
  (call $ta_blit_from_string (local.get $s) (local.get $pos) (local.get $buf)
    (local.get $curr) (local.get $len))
  (struct.set $channel $curr (local.get $ch)
    (i32.add (local.get $curr) (local.get $len)))
  (if (i32.ge_u (local.get $len) (local.get $free))
  ...
  (loop $loop
    (if (i32.gt_u (local.get $len) (i32.const 0))
      (then
        (local.set $written
          (call $caml_putblock (ref.cast (ref $channel) (local.get $ch))
            (ref.cast (ref $string) (local.get $s)) (local.get $pos)
            (local.get $len)))
        (local.set $pos (i32.add (local.get $pos) (local.get $written)))
        (local.set $len (i32.sub (local.get $len) (local.get $written)))
        (br $loop))))
...
(func $caml_putch (param $ch (ref $channel)) (param $c i32)
  (local $curr i32)
  (if
    (i32.ge_u (struct.get $channel $curr (local.get $ch))
      (struct.get $channel $size (local.get $ch)))
    (then (drop (call $caml_flush_partial (local.get $ch)))))
  (local.set $curr (struct.get $channel $curr (local.get $ch)))
...
(func (export "caml_ml_output_int")
  ...
  (call $caml_putch (local.get $ch) (i32.shr_u (local.get $n) (i32.const 24)))
  (call $caml_putch (local.get $ch) (i32.shr_u (local.get $n) (i32.const 16)))
  (call $caml_putch (local.get $ch) (i32.shr_u (local.get $n) (i32.const 8)))
...
(func (export "caml_ml_set_channel_fd") (param (ref eq) i32)
  (struct.set $channel $fd (ref.cast (ref $channel) (local.get 0))
    (local.get 1))
)
