(module
  ...
  (import "bindings" "open"
    (func $open_2 (param anyref i32 i32) (result i32))
  )
  ...
  (import "bindings" "ta_new"
    (func $ta_new (param i32) (result (ref extern)))
  )
  ...
  (import "bindings" "map_delete"
    (func $map_delete (param (ref extern) i32))
  )
      ...
      (then
        (if (i32.gt_u (local.get $avail) (i32.const 0))
          ...
          (else
            ...
            (if (i32.gt_u (local.get $len) (local.get $nread))
              (then (local.set $len (local.get $nread))))))))
    (call $ta_blit_to_string (local.get $buf) (local.get $curr)
  ...
  (func $caml_getch (param $ch (ref $channel)) (result i32)
    ...
    (local.set $curr (struct.get $channel $curr (local.get $ch)))
    (if
      (i32.ge_u (local.get $curr) (struct.get $channel $max (local.get $ch)))
  ...
  (func $caml_ml_input_scan_line (export "caml_ml_input_scan_line")
    ...
    (loop $loop
      (if
        (i32.ge_u (local.get $p) (struct.get $channel $max (local.get $ch)))
        (then
  ...
  (func $caml_flush_partial (param $ch (ref $channel)) (result i32)
    ...
    (if (i32.gt_u (local.get $towrite) (i32.const 0))
      (then
              ...
              (if (result i32)
                ...
                (else
                  (call $write' (local.get $fd) (local.get $buf)
                    (i32.const 0) (local.get $towrite) (ref.null noextern))))))
          (catch $javascript_exception (call $caml_handle_sys_error)))
        (struct.set $fd_offset $offset (local.get $fd_offset)
        ...
        (if (i32.gt_u (local.get $towrite) (i32.const 0))
          (then
            (call $ta_copy (local.get $buf) (i32.const 0)
              (local.get $written) (local.get $towrite))))
  ...
  (func $caml_putblock
    ...
    (local.set $buf (struct.get $channel $buffer (local.get $ch)))
    (call $ta_blit_from_string (local.get $s) (local.get $pos)
