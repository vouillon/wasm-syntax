(module
  ...
  (func $pop_compare_stack
    ...
    (local.set $p
      (array.get $int_array
        (struct.get $compare_stack $f_4 (local.get $stack)) (local.get $i)))
  ...
  (func $push_compare_stack
    ...
    (local.set $i
      (i32.add (struct.get $compare_stack $f (local.get $stack))
        (i32.const 1)))
    (local.set $len
      ...
      (then
        ...
        (local.set $stack'
          (struct.new $compare_stack (local.get $i)
            (array.new $block_array (global.get $dummy_block)
              (local.get $len'))
            (array.new $block_array (global.get $dummy_block)
              (local.get $len'))
