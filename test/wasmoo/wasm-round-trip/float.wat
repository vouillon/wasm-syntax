(module
  ...
  (type $chars (array i8))
  (global $infinity (ref $chars) (array.new_fixed $chars 0))
  (global $nan_2 (ref $chars) (array.new_fixed $chars 0))
      ...
      (block $sign (result (ref $string))
        ...
        (if
          ...
          (then
            ...
            (local.set $m
              (i64.and (local.get $m)
                (i64.xor (i64.sub (i64.const 0) (i64.const 1))
                  (local.get $mask))))
            (if
  ...
  (global $inf_2 (ref $chars) (array.new_fixed $chars 0))
  (func $caml_format_float (export "caml_format_float")
    ...
    (call $parse_format (ref.cast (ref $string) (local.get $x)))
    (local.set $uppercase)
    (local.set $conversion)
    (local.set $precision)
          ...
          (then
            ...
            (if (i32.eq (local.get $c) (i32.const 46))
              (then
                ...
                (br $parse)))
            (if
              (i32.or (i32.eq (local.get $c) (i32.const 80))
          ...
          (if (i32.lt_u (local.get $i) (local.get $len))
            (then
              (local.set $c
                (array.get_u $string (local.get $s) (local.get $i)))
              (local.set $i (i32.add (local.get $i) (i32.const 1)))
              (if (i32.ne (local.get $c) (i32.const 95))
                (then
    ...
    (block $error
      ...
      (if (i32.eq (i32.add (local.get $i) (i32.const 3)) (local.get $len))
        (then
          (local.set $c (array.get_u $string (local.get $s) (local.get $i)))
          (if
            (i32.eq (i32.and (local.get $c) (i32.const 0xdf)) (i32.const 78))
            (then
              (local.set $i (i32.add (local.get $i) (i32.const 1)))
              (local.set $c
                (array.get_u $string (local.get $s) (local.get $i)))
          ...
          (if
            (i32.eq (i32.and (local.get $c) (i32.const 0xdf)) (i32.const 73))
            (then
              (local.set $i (i32.add (local.get $i) (i32.const 1)))
              (local.set $c
                (array.get_u $string (local.get $s) (local.get $i)))
  ...
  (func $caml_modf_float (export "caml_modf_float")
    ...
    (local.set $x_2
      (struct.get $float $f (ref.cast (ref $float) (local.get $x))))
    (local.set $a (f64.abs (local.get $x_2)))
      ...
      (then
        (if (f64.lt (local.get $a) (f64.const inf))
          (then
            ...
            (local.set $i (f64.copysign (local.get $i) (local.get $x_2)))
            (local.set $f (f64.copysign (local.get $f) (local.get $x_2))))
          (else
            (local.set $i (local.get $x_2))
            (local.set $f (f64.copysign (f64.const 0) (local.get $x_2))))))
      (else (local.set $i (local.get $x_2)) (local.set $f (local.get $x_2))))
              ...
              (then
                ...
                (if
                  (i32.lt_s (local.get $n)
                    (i32.sub (i32.const 0) (i32.const 1022)))
                  (then
                    (local.set $n (i32.sub (i32.const 0) (i32.const 1022)))))))))))
    (f64.mul (local.get $x)
      (f64.reinterpret_i64
        (i64.shl
          (i64.add (i64.extend_i32_s (local.get $n)) (i64.const 0x3ff))
          (i64.const 52))))
  )
  (func $caml_ldexp_float (export "caml_ldexp_float")
    (param $x f64) (param $i (ref eq)) (result f64)
    (call $ldexp (local.get $x)
  ...
  (func $caml_fma_float (export "caml_fma_float")
    ...
    (local $x_3 i64) (local $x_4 i64) (local $x_5 i64) (local $x_6 i64)
    (local $x_7 i64) (local $x_8 i32) (local $x_9 i32) (local $x_10 f64)
    (local $x_11 f64) (local $x_12 f64) (local $x_13 f64) (local $x_14 f64)
    (local.set $x
      (struct.get $float $f (ref.cast (ref $float) (local.get $vx))))
    (local.set $y
      (struct.get $float $f (ref.cast (ref $float) (local.get $vy))))
    (local.set $z
      (struct.get $float $f (ref.cast (ref $float) (local.get $vz))))
    (local.set $x_6
      (i64.add
        (local.tee $x_3
          (i64.and
    ...
    (block $l_2
      ...
      (block $cont
        (br_if $cont (i64.eq (local.get $x_3) (i64.const 2047)))
        (br_if $cont (i64.eq (local.get $x_5) (i64.const 2047)))
      ...
      (block $cont
        (block $then
          (br_if $then (i64.eq (local.get $x_5) (i64.const 2047)))
          (br_if $then (i64.eq (local.get $x_3) (i64.const 2047)))
      ...
      (block $cont
        ...
        (block $cont2
          (br_if $cont2 (i64.lt_u (local.get $x_7) (i64.const 3)))
          ...
          (block $l_5
            ...
            (local.set $x
              (select (f64.mul (local.get $x) (f64.const 0x1p-53))
                (local.get $x)
                (local.tee $x_8 (i64.gt_u (local.get $x_5) (local.get $x_3)))))
            (local.set $y
              (select (local.get $y)
                      ...
                      (f64.sub (local.get $z)
                        (local.tee $x_12
                          (f64.sub
                            ...
                            (local.get $y))))
                      (f64.sub (local.get $y)
                        (f64.sub (local.get $x) (local.get $x_12)))))
                  (local.tee $y (f64.add (local.get $x_10) (local.get $y))))))
            (f64.const 0)))
        (br_if $cont
          (i32.and
            (i32.wrap_i64
              (local.tee $x_3 (i64.reinterpret_f64 (local.get $y))))
            (i32.const 1)))
        (local.set $y
          (f64.reinterpret_i64
            (i64.add
              (select (i64.const 1) (i64.sub (i64.const 0) (i64.const 1))
