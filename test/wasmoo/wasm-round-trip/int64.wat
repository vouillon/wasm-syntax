(module
  ...
  (global $int64_ops (export "int64_ops") (ref $custom_operations)
    (struct.new $custom_operations
      (array.new_fixed $string 2 (i32.const 95) (i32.const 106))
  ...
  (func $int64_cmp
    ...
    (local.set $i2
      (struct.get $int64 $f_2 (ref.cast (ref $int64) (local.get $v2))))
  ...
  (global $INT64_ERRMSG (ref $string)
    (array.new_fixed $string 15 (i32.const 73) (i32.const 110)
      (i32.const 116) (i32.const 54) (i32.const 52) (i32.const 46)
    ...
    (if (local.get $signedness)
      (then
        (if (i32.gt_s (local.get $sign) (i32.const 0))
          (then
            (if
              (i64.ge_u (local.get $res)
                (i64.shl (i64.const 1) (i64.const 63)))
              (then (call $caml_failwith (local.get $errmsg)))))
          (else
            (if
              (i64.gt_u (local.get $res)
                (i64.shl (i64.const 1) (i64.const 63)))
              (then (call $caml_failwith (local.get $errmsg))))))))
  ...
  (func $caml_int64_format (export "caml_int64_format")
    ...
    (local $uppercase i32) (local $negative i32) (local $i i32)
    (local $n i64) (local $chars (ref $chars))
    (local.set $s (ref.cast (ref $string) (local.get $x)))
    (local.set $d
      (struct.get $int64 $f_2 (ref.cast (ref $int64) (local.get $x_2))))
