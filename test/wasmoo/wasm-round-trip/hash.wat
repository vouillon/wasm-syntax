(module
  ...
  (func $caml_hash_mix_double (export "caml_hash_mix_double")
    ...
    (if
      (i64.eq (i64.and (local.get $i) (i64.const 0x7FF0000000000000))
        (i64.const 0x7ff0000000000000))
      (then
        (if
          (i64.ne (i64.and (local.get $i) (i64.const 0xFFFFFFFFFFFFF))
            (i64.const 0))
  ...
  (func $caml_hash_mix_float (export "caml_hash_mix_float")
    ...
    (if
      (i32.eq (i32.and (local.get $i) (i32.const 0x7F800000))
        (i32.const 0x7F800000))
      (then
        (if
          (i32.ne (i32.and (local.get $i) (i32.const 0x7FFFFF)) (i32.const 0))
  ...
  (func $caml_hash (export "caml_hash")
    ...
    (local $i i32) (local $len i32) (local $tag_2 i32) (local $str anyref)
    ...
    (array.set $block (global.get $caml_hash_queue) (i32.const 0)
      (local.get $obj))
    (local.set $rd (i32.const 0))
    (local.set $wr (i32.const 1))
    (loop $loop
      (if
        (i32.and (i32.lt_u (local.get $rd) (local.get $wr))
          (i32.gt_u (local.get $num) (i32.const 0)))
        (then
          (local.set $v
            (array.get $block (global.get $caml_hash_queue) (local.get $rd)))
          ...
          (block $again
            ...
            (drop
              (block $not_block (result (ref eq))
                ...
                (if (i32.eq (local.get $tag_2) (global.get $forward_tag))
                  (then
                    (local.set $i (i32.const 0))
                    (loop $forward
                      (local.set $v
                    ...
                    (call_ref $hash (local.get $v)
                      (br_on_null $loop
                        (struct.get $custom_operations $hash
                          (struct.get $custom $f
                            (br_on_cast_fail $not_custom (ref eq)
                              (ref $custom) (local.get $v))))))))
                (local.set $num (i32.sub (local.get $num) (i32.const 1)))
                (br $loop)))
            (drop
                ...
                (local.set $str
                  (struct.get $js $f
                    (br_on_cast_fail $not_jsstring (ref eq) (ref $js)
                      (local.get $v))))
                (drop
                  (br_if $not_jsstring (ref.i31 (i32.const 0))
      ...
      (i32.and
        (call $caml_hash_mix_final
          (call $caml_hash_mix_string
            ...
            (ref.cast (ref $string) (local.get $x_2))))
        (i32.const 0x3FFFFFFF)))
  )
)
