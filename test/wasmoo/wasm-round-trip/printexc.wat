  ...
  (if (result (ref eq))
    ...
    (then
            ...
            (br_if $default
              (i32.eqz
                (call $caml_is_special_exception
                  (array.get $block (local.get $exn) (i32.const 1)))))
            (local.set $v (array.get $block (local.get $exn) (i32.const 2)))
            (br_if $default (i32.eqz (ref.test (ref $block) (local.get $v))))
      ...
      (array.copy $string $string (local.get $s) (i32.const 0)
        (struct.get $buffer $f_2 (local.get $buf)) (i32.const 0)
        (struct.get $buffer $f (local.get $buf)))
      (local.get $s))
    (else (array.get $block (local.get $exn) (i32.const 1))))
)
