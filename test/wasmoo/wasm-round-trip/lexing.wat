...
(func $caml_lex_engine (export "caml_lex_engine")
  ...
  (local $state i32) (local $buffer (ref $string)) (local $vpos (ref eq))
  (local $action (ref eq)) (local $pos i32) (local $base i32)
  (local $backtrk i32) (local $lex_base_2 (ref $string))
  (local $lex_backtrk_2 (ref $string)) (local $lex_check_2 (ref $string))
  (local $lex_check_code_2 (ref $string)) (local $lex_trans_2 (ref $string))
  (local $lex_default_2 (ref $string))
  (local.set $tbl (ref.cast (ref $block) (local.get $vtbl)))
  (local.set $lexbuf (ref.cast (ref $block) (local.get $vlexbuf)))
  (local.set $state (i31.get_s (ref.cast (ref i31) (local.get $start_state))))
  (local.set $buffer
    (ref.cast (ref $string)
  ...
  (loop $loop
    ...
    (if
      (i32.eq
        (call $get (local.get $lex_check_2)
          (i32.add (local.get $base) (local.get $c)))
        (local.get $state))
      (then
        (local.set $state
          (call $get (local.get $lex_trans_2)
            (i32.add (local.get $base) (local.get $c)))))
      (else
        (local.set $state
          (call $get (local.get $lex_default_2) (local.get $state)))))
...
(func $caml_new_lex_engine (export "caml_new_lex_engine")
  ...
  (param $vlexbuf (ref eq)) (result (ref eq))
  (local $tbl (ref $block)) (local $lexbuf (ref $block)) (local $c i32)
  (local $state i32) (local $pstate i32) (local $buffer (ref $string))
  (local $vpos (ref eq)) (local $action (ref eq)) (local $pos i32)
  (local $base i32) (local $backtrk i32) (local $pc_off i32)
  (local $base_code i32) (local $lex_code_2 (ref $string))
  (local $lex_base_2 (ref $string)) (local $lex_base_code_2 (ref $string))
  (local $lex_backtrk_2 (ref $string))
  (local $lex_backtrk_code_2 (ref $string))
  (local $lex_check_2 (ref $string)) (local $lex_check_code_2 (ref $string))
  (local $lex_trans_2 (ref $string)) (local $lex_trans_code_2 (ref $string))
  (local $lex_default_2 (ref $string))
  (local $lex_default_code_2 (ref $string))
  (local.set $tbl (ref.cast (ref $block) (local.get $vtbl)))
  (local.set $lexbuf (ref.cast (ref $block) (local.get $vlexbuf)))
  (local.set $state (i31.get_s (ref.cast (ref i31) (local.get $start_state))))
  (local.set $buffer
  ...
  (loop $loop
    ...
    (if (i32.ge_s (local.get $backtrk) (i32.const 0))
      (then
        (local.set $pc_off
          (call $get (local.get $lex_backtrk_code_2) (local.get $state)))
        (call $run_tag (local.get $lex_code_2) (local.get $pc_off)
          (local.get $lexbuf))
        (array.set $block (local.get $lexbuf) (global.get $lex_last_pos)
          (array.get $block (local.get $lexbuf) (global.get $lex_curr_pos)))
    ...
    (local.set $pc_off
      (if (result i32)
        (i32.eq
          (call $get (local.get $lex_check_code_2)
            (i32.add (local.get $base_code) (local.get $c)))
          (local.get $pstate))
        (then
          (call $get (local.get $lex_trans_code_2)
            (i32.add (local.get $base_code) (local.get $c))))
        (else (call $get (local.get $lex_default_code_2) (local.get $pstate)))))
    (call $run_mem (local.get $lex_code_2) (local.get $pc_off)
      (local.get $lexbuf)
      (array.get $block (local.get $lexbuf) (global.get $lex_curr_pos)))
    (if (i32.eq (local.get $c) (i32.const 256))
      (then
